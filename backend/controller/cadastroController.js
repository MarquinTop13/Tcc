...
endpoint.post("/registro", async (req, res) => {
    const { nome, email, senha, palavra, confirmarSenha } = req.body;

    if (!nome || !email || !senha || !confirmarSenha || !palavra) {
    ...
    const id = await Repository.salvarUsuario({ nome, email, senha, palavra });
